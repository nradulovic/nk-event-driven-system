...
int main(void)
{
    ...
    struct blinky2_workspace sm__blink_workspace1 =
    {
        .period_ms = 1000, 
        .instance = 0
    };
    struct blinky2_workspace sm__blink_workspace2 =
    {
        .period_ms = 500, 
        .instance = 1
    };
    ...
    error = eds__agent_create(blinky2_initial_state, &sm__blink_workspace1, NULL, &sm_blink_agent1);
    assert(error == EDS__ERROR_NONE);

    error = eds__agent_create(blinky2_initial_state, &sm__blink_workspace2, NULL, &sm_blink_agent2);
    assert(error == EDS__ERROR_NONE);
